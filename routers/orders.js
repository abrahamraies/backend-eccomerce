...
router.post(`/`, async (req, res) => {
  ...
  const ordersItemsIdsResolved = await orderItemsIds;

  const toatlPrices = await Promise.all(ordersItemsIdsResolved.map( async (orderItemId) => {
    const orderItem = await OrderItem.findById(orderItemId).populate('product','price');
    const totalPrice = orderItem.product.price * orderItem.quantity;
    return totalPrice;
  }))

  const totalPrice = toatlPrices.reduce((a,b) => a + b, 0);
  
  let order = new Order({
    orderItems: ordersItemsIdsResolved,
